// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace Rhyous.DataMigration.AdvisorsAssistant.Models;

public partial class AacalEventException
{
    public int IKeyId { get; set; }

    public int IKeyEvent { get; set; }

    public int IInstance { get; set; }

    public bool BFlagRemove { get; set; }

    public bool BFlagCompleted { get; set; }

    public bool BFlagRolloverNotification { get; set; }

    public DateTime? DOriginalBegin { get; set; }

    public DateTime? DNewBegin { get; set; }

    public DateTime? DNewEnd { get; set; }

    public DateTime? DNewDue { get; set; }

    public DateTime? DJustDate { get; set; }

    public string CWhoChange { get; set; }

    public DateTime? DLastChange { get; set; }

    public Guid GUidProcess { get; set; }

    public string CWhoAdded { get; set; }

    public DateTime? DWhenAdded { get; set; }

    public string CSerialId { get; set; }

    public Guid GUid { get; set; }

    public int IRecVersion { get; set; }
}